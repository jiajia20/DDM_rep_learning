...
class Game(IteratedExperiment):
    ...
    def run_participant_prepare(self, participant, condition, context):
        self.agents = [pyibl.Agent(str(i), default_utility=PREPOPULATED_VALUE)
    ...
    def run_participant_run(self, round, participant, condition, context):
        ...
        for p in random.choice(PAIRINGS[condition]):
            choices = tuple(self.agents[i].choose(*"AB") for i in p)
            if choices == ("A", "A"):
                result_AA += 1 / (PAIRS * GROUPS)
            # elif choices == ("A", "B"):
            #     result_AB += 1 / (PAIRS * GROUPS)
            # elif choices == ("B", "A"):
            #     result_BA += 1 / (PAIRS * GROUPS)
            # elif choices == ("B", "B"):
            #     result_BB += 1 / (PAIRS * GROUPS)
